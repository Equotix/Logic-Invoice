...
class ModelContentEmailTemplate extends Model {
    public function getEmailTemplateByType($type) {
        ...
        if (!$email_template_data) {
            ...
            if ($query->num_rows) {
                $variables_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "email_template_type WHERE type LIKE '" . $this->db->escape($type) . "%' LIMIT 1");
    ...
    public function send($data, $type, $language_id = false) {
        ...
        if ($template_info) {
            ...
            $html = $this->load->view('mail/general.tpl', $template_data);
